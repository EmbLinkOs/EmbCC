...
| **CX5** (in progress) exceptions: unwind tables and LSDAs g++'s runtime reads, `throw`/`try`/`catch`, destructors during unwinding, `noexcept` — thrown and caught across EmbCC and g++ code both ways | ✓ | ✓ |
