...
# (tests/golden/arm64-asm.s), and every entry of the assembler's own
...
$CC -std=c99 -Wall -Wextra -Werror -o "$out/check" \
    tools/a64check/a64asmcheck.c src/asm/asm_arm64.c src/asm/emit_arm64.c \
    src/asm/emit.c src/driver/util.c

{ grep -v '^//' tests/golden/arm64-asm.s; "$out/check" --vocabulary; } > "$out/lines"
